...
function kube_proxy_synced() {
    oc exec -n kube-proxy-test kube-proxy -- curl -s http://127.0.0.1:10249/metrics || echo OOPS
    oc exec -n kube-proxy-test kube-proxy -- curl -s http://127.0.0.1:10249/metrics |& grep -q '^kubeproxy_sync_proxy_rules_duration_seconds_count [^0]'
}
synced=false
...
if [[ "${synced}" != true ]]; then
    ...
    echo "metrics:"
    oc exec -n kube-proxy-test kube-proxy -- curl -s http://127.0.0.1:10249/metrics |& indent
